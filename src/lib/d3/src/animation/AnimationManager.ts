	...
	enroll(obj) {
		...
		obj._setObject = (options) => {
			...
			if (t) {
				...
				//this.translateY(c.y);
				//this.translateZ(c.z);
				options.position = obj.coordinates;
			}
			...
			if (wt) {
				...
				obj.translateY(wt.y);
				obj.translateZ(wt.z);
				let p = utils.unprojectFromWorld(obj.position);
				obj.coordinates = options.position = p;
			}
			...
			for (let i = object.animationQueue.length - 1; i >= 0; i--) {
				...
				if (expiring) {
					...
					if (item.type === 'followPath') {
						...
						let objectState: any = { worldCoordinates: position };
