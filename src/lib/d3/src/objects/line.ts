 ...
 */
	...
	class LineSegmentsGeometry extends THREE.InstancedBufferGeometry {
		...
		computeBoundingBox() {
			...
			const start:any = this.attributes.instanceStart;
			const end:any = this.attributes.instanceEnd;
 ...
 */
	...
	class WireframeGeometry2 extends LineSegmentsGeometry {
		type: string = 'WireframeGeometry2';
		isWireframeGeometry2: boolean;
		fromWireframeGeometry: any;
 ...
 */
	...
	class LineSegments2 extends THREE.Mesh {
		...
		constructor(geometry:any = new LineSegmentsGeometry(), material = new LineMaterial({
		...
		raycast(raycaster, intersects) {
			...
			const instanceStart:any = geometry.attributes.instanceStart;
			const instanceEnd:any = geometry.attributes.instanceEnd; // camera forward is negative
			...
			_box1.copy(geometry.boundingBox).applyMatrix4(matrixWorld);

			const distanceToBox = Math.max(camera.near, _box1.distanceToPoint(ray.origin)); // get the w component to scale the world space line width
			...
			_box1.max.x += boxMargin;
			_box1.max.y += boxMargin;
			_box1.max.z += boxMargin;
			_box1.min.x -= boxMargin;
			_box1.min.y -= boxMargin;
			_box1.min.z -= boxMargin;

			if (raycaster.ray.intersectsBox(_box1) === false) {
	...
	class Wireframe extends THREE.Mesh {
		...
		computeLineDistances() {
			...
			for (let i = 0, j = 0, l = instanceStart.count; i < l; i++, j += 2) {
				...
				_start1.fromBufferAttribute(instanceStart, i);

				_end1.fromBufferAttribute(instanceEnd, i);

				lineDistances[j] = j === 0 ? 0 : lineDistances[j - 1];
				lineDistances[j + 1] = lineDistances[j] + _start1.distanceTo(_end1);
