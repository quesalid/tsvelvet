...
function convertArrayBufferToString(buffer, from=undefined, to=undefined) {
...
function inject(a1, index, a2) {
	...
	class FBXLoader extends THREE.Loader {
		...
		load(url, onLoad, onProgress, onError) {
			...
			loader.load(url, function (buffer) {
				console.log("FBXLoader",url)
				try {
					console.log("FBXLoader ---------------------------> try to parse")
	...
	class FBXTreeParser {
		...
		parse() {
			//console.log("FBXTreeParser")
			connections = this.parseConnections();
			//console.log("FBXTreeParser Connections")
			const images = this.parseImages();
			//console.log("FBXTreeParser Images")
			const textures = this.parseTextures(images);
			//console.log("FBXTreeParser Textures")
			const materials = this.parseMaterials(textures);
			//console.log("FBXTreeParser Materials")
			const deformers = this.parseDeformers();
			//console.log("FBXTreeParser Deformers")
			const geometryMap = new GeometryParser().parse(deformers);
			//console.log("FBXTreeParser Geomrtry")
			this.parseScene(deformers, geometryMap, materials);
			//console.log("FBXTreeParser Scene", sceneGraph)
			return sceneGraph;
	...
	class BinaryParser {
		...
		parse(buffer) {
			...
			while (!this.endOfContent(reader)) {
				...
				const node:any = this.parseNode(reader, version);
		...
		parseNode(reader, version) {
			...
			const node:any = {}; // The first three data sizes depends on version.
...
class BinaryReader {
	...
	littleEndian: boolean
		constructor(buffer, littleEndian=undefined) {
