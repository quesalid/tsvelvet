...
class Objects {
	...
	static _defaults: any = _defaults;
	static geometries: any = geometries;
	...
	line(obj) {
		...
		let straightProject = utils.lnglatsToWorld(obj.geometry);
		let normalized = utils.normalizeVertices(straightProject);

		//flatten array for buffergeometry
		let flattenedArray = utils.flattenVectors(normalized.vertices);

		let positions = new Float32Array(flattenedArray); // 3 vertices per point
		let geometry = new THREE.BufferGeometry();
		geometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));

		// material
		let material = new THREE.LineBasicMaterial({ color: 0xff0000, linewidth: 21 });
	...
	_addMethods(obj, isStatic) {
		...
		if (isStatic) {
			...
			obj.calculateAdjustedPosition =  (lnglat, xyz, inverse) =>{
			...
			obj.drawBoundingBox = () => {
				...
				let boxModel = new THREE.Box3Helper(bb, Objects._defaults.colors.yellow);
				...
				bb2.max.z = bb2.min.z;
				let boxShadow = new THREE.Box3Helper(bb2, Objects._defaults.colors.black);
			...
			obj.addLabel =  (HTMLElement, visible, center, height) => {
				if (HTMLElement) {
			...
			obj.drawLabelHTML =  (HTMLElement, visible = false, center = obj.anchor, height = 0.5) => {
				let divLabel = root.drawLabelHTML(HTMLElement, Objects._defaults.label.cssClass);
			...
			obj.addTooltip =  (tooltipText, mapboxStyle, center, custom = true, height = 1) => {
			...
			obj.addCSS2D = (element, objName, center = obj.anchor, height = 1) =>{
				if (element) {
					const box = obj.box3();
				...
				set(value) {
					if (value) {
						if (!obj.selected) {
							...
							if (obj.boxGroup) {
								obj.boundingBox.material = Objects._defaults.materials.boxOverMaterial;
								obj.boundingBox.parent.visible = true;
								obj.boundingBox.layers.enable(1);
								obj.boundingBoxShadow.layers.enable(1);
							}
		...
		obj.duplicate = (options) => {
			...
			root._addMethods(dupe,false); // add methods
	...
	_makeGroup(obj, options) {
		let projScaleGroup:any = new THREE.Group();
	...
	drawTooltip(tooltipText, mapboxStyle = false) {
		if (tooltipText) {
			let divToolTip;
			if (mapboxStyle) {
				...
				divToolTip.className = Objects._defaults.tooltip.cssClass;
	...
	drawLabelHTML(HTMLElement, cssClass) {
		let div = document.createElement('div');
