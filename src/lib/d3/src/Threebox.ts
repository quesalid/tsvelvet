...
class Threebox {
	...
	setSunlight(newDate = new Date(), coords=undefined) {
	...
	getSunSky(date, sunPos=undefined) {
		if (!sunPos) {
	...
	loadObj(options, cb,) {
		...
		if (options.clone === false) {
			return new Promise(
				async (resolve) => {
					loader(options, cb, this.map, async (obj) => {
						resolve(obj);
					});
				});
		}
...
//get the center point of a feature
	getFeatureCentergetFeatureCenter(feature, obj, level) {
